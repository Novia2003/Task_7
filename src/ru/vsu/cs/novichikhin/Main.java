...
public class Main {
    ...
    static void printIndexDesiredElement(int index, int[] array, int n) {
        if (index != -1) {
            System.out.printf("Минимальный элемент - элемент с индексом %s, расположенный между последним ", index);
            System.out.printf("максимальным элементом массива и %s-ым четным элементом и равен %s", n, array[index]);
        } else {
